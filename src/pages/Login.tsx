...
import clinicLogo from "@/assets/clinic-logo-pdf.ico";
import { useAuth } from "@/contexts/AuthContext"; // <- fixed import

const Login = () => {
  const navigate = useNavigate();
  const { setUser } = useAuth(); // <- update context
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.token) {
        if (data.user.role !== "N") {
          toast.error("Unauthorized role. Only Nurses can log in.");
          setIsLoading(false);
          return;
        }
        ...
        navigate("/dashboard"); // ✅ navigate after context updated
      } else {
